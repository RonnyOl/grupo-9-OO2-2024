...
public class StockService implements IStockService {
    ...
    public StockService(IStockRepository StockRepository,IOrdenDeCompraService ordendDeCompraService,IProveedorService proveedorService) {
	...
	public boolean validarRabastecer(int id) {
		...
		boolean reabastecer;
		if (stock.isPresent() && (stock.get().getCantidadActual() <= stock.get().getPuntoMinimoDeStock() ) ) {
			reabastecer=true;
		}else {
			reabastecer=false;
		}
	...
	public boolean cambiarEstadoDeReabastecer(int id, boolean estado) {
		...
		if (estado==true)this.verificarYGenerarOrdenDeCompra();
	...
	public boolean verificarYGenerarOrdenDeCompra() {
		...
		if (stockAReabastecer.isPresent()) {
			//LA CANTIDAD A COMPRAR ES IGUAL AL PUNTO MINIMO DE STOCK DETERMINADO PARA ESE PRODUCTO, 
			int cantidadAComprar=stockAReabastecer.get().getPuntoMinimoDeStock(); //ASI SE SABE AL 100 QUE SATISFACE LA CONDICION
			
			//SE TRAEN LOS PROVEEDORES EXISTENTES EN LA BD Y SE LE ASIGNA UNO ALEATORIO A LA ORDEN DE COMPRA
			...
			OrdenDeCompra nuevaOrden = new OrdenDeCompra(cantidadAComprar,stockAReabastecer.get(),null,proveedorAsignado,false);
